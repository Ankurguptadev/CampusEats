...
def seed():
    ...
    demo = [
        ("Curry Point", "North Campus", "budget", True),
        ("Sushi Central", "South Campus", "premium", True),
        ("Old Kitchen", "North Campus", "budget", False),
    ]
    for name, area, tier, active in demo:
        r = Restaurant(_next_restaurant_id, name, area, tier, active)
        _restaurants[r.id] = r
        _next_restaurant_id += 1
    create_item(1, "Veg Thali", 8000, 4800, True)
    create_item(1, "Cold Coffee", 6000, 2400, True)
    _menu_items[2].available = False
...
def list_restaurants(area=None, sort=None, order="asc", page=1, limit=10):
    vals = list(_restaurants.values())
    if area:
        vals = [r for r in vals if r.area.lower() == area.lower()]
    if sort in {"name", "area", "tier", "id"}:
        vals.sort(key=lambda r: getattr(r, sort), reverse=(order == "desc"))
    start = (page - 1) * limit
    return vals[start:start + limit]

def restaurant_count(area=None):
    vals = list(_restaurants.values())
    if area:
        vals = [r for r in vals if r.area.lower() == area.lower()]
    return len(vals)
...
def list_items_for_restaurant(rid, sort=None, order="asc", page=1, limit=10):
    vals = [m for m in _menu_items.values() if m.restaurant_id == rid]
    if sort in {"name", "price", "id"}:
        vals.sort(key=lambda m: getattr(m, sort), reverse=(order == "desc"))
    start = (page - 1) * limit
    return vals[start:start + limit]

def item_count_for_restaurant(rid):
    return sum(1 for m in _menu_items.values() if m.restaurant_id == rid)

def item_name_exists(restaurant_id, name, except_id=None):
    return any(
        m.restaurant_id == restaurant_id and
        m.id != except_id and
...
def delete_item(iid):
    m = _menu_items.pop(iid, None)
    if m and m.idempotency_key:
        _by_key.pop(m.idempotency_key, None)
    return m
